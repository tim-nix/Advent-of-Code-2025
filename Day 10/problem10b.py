...
def findFewest(configuration):
   ...
   states = [(0, [ 0 for i in range(len(joltages)) ])]
   visited = set()

   # Keep searching until the current state matches
   # the final state
   while states[0][1] != joltages:
      ...
      for w in wiring:
         ...
         # Ignore configurations already seen or
         # if a current joltage is too big and
         # update the states queue
         if all([ next_state[i] <= joltages[i] ] for i in range(len(joltages))):
...
if __name__ == '__main__':
   ...
   for c in range(len(configs)):
      print(c)
      button_presses += findFewest(configs[c])
